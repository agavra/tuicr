...
- `Confirm` - Y/N confirmation dialog; `ConfirmAction` picks the prompt and what "no" means (`CopyAndQuit` only asks whether to export on the way out, so "no" still quits; `Quit` — the `confirm_quit = true` prompt — asks whether to leave at all, so "no" returns to the review)
