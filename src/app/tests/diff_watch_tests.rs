...
/// leaving the in-flight record set would block every later tick, and the
/// watcher would be dead for the rest of the session while the status bar
/// still showed the watching badge. This manufactures the dropped sender
/// directly, since nothing else can.
#[test]
