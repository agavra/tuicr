...
impl App {
    ...
    fn spawn_diff_watch_reload(&mut self) {
        ...
        std::thread::spawn(move || {
            ...
            match outcome {
                ...
                Err(e) => reporter.answer(normalize_diff_watch_result(Err(e)), None),
            }
        });
    }
    ...
    fn poll_diff_watch_reload_events(&mut self) -> bool {
        ...
        let event = match in_flight.rx.try_recv() {
            ...
            // tick while the status bar still claims the diff is watched.
        ...
        let commit_pane_changed = self.apply_fetched_commits(commits);
        ...
        let diff_changed = match result {
            ...
            Ok(Some(diff_files)) => {
                self.last_diff_watch_error = None;
                // The worker compared against the diff as it stood when it was
                // spawned. A `:e` landing in between can already have applied
                // the same content, so compare against what is on screen now.
                // Applying anyway would clear expanded gaps and print a second
                // "Reloaded" message with nothing new to show.
                if diff_files_fingerprint(&diff_files) == diff_files_fingerprint(&self.diff_files) {
                    ...
                    if invalidated > 0 {
                        ...
                        self.set_message(format!("Reloaded {count} files"));
                    }
                    true
                }
            }
    ...
    fn apply_fetched_commits(&mut self, fetched: Option<Vec<CommitInfo>>) -> bool {
        let Some(rebuilt) = self.rebuilt_commit_pane(fetched) else {
            return false;
        };
    ...
    ///
    /// Two ways to get `None`, and the first is the one worth naming: an empty
    /// answer means "nothing to say", never "delete every row".
    /// `VcsBackend::get_recent_commits` defaults to `Ok(Vec::new())` for
    /// unsupported rather than an error (`src/vcs/traits.rs`), so without that
    /// filter a future backend that skipped it would wipe the pane. The second
    /// is simply that the rebuilt pane matches what is already on screen.
    fn rebuilt_commit_pane(&self, fetched: Option<Vec<CommitInfo>>) -> Option<Vec<CommitInfo>> {
        let fetched = fetched.filter(|commits| !commits.is_empty())?;
        let rebuilt = Self::commit_pane_rows(&self.review_commits, fetched);
    ...
    /// The commit pane after a refresh: the synthetic rows the pane already
    /// shows, followed by the freshly fetched commits.
    ///
    /// The pane can open with up to two synthetic rows above the real commits,
    /// one standing for staged changes and one for unstaged
    /// (`staged_commit_entry`, `unstaged_commit_entry`). They are always a
    /// prefix, which is what `special_commit_count` relies on.
    /// `get_recent_commits` does not know about them, so installing its answer
    /// verbatim would delete both.
    ///
    /// Carrying the existing rows forward rather than rebuilding them from
    /// change status keeps their order as the user has been seeing it. The two
    /// startup paths disagree about that order: `App::new` builds staged first
    /// (`src/app/init.rs`) and `enter_target_selector` builds unstaged first
    /// (`src/app/commits.rs`). Re-deriving here would reorder the pane for
    /// whichever half of them the user opened from. It also means a side that
    /// becomes staged mid-session gains no row until the review target is
    /// reopened; see the `:commits` path, which does rebuild from status.
    fn commit_pane_rows(current: &[CommitInfo], fetched: Vec<CommitInfo>) -> Vec<CommitInfo> {
        let mut rows: Vec<CommitInfo> = current
            .iter()
            .take_while(|commit| Self::is_special_commit(commit))
            .cloned()
            .collect();
