...
/// Parse unified diff lines into DiffFile structures plus highlight jobs.
///
...
pub fn parse_unified_diff_lines<'a, I>(diff_lines: I, format: DiffFormat) -> Result<DiffWithJobs>
where
...
"#;
        ...
        let (files, _) = parse_unified_diff(diff, DiffFormat::GitStyle).unwrap();
    ...
    fn should_parse_unified_diff_from_streamed_lines() {
        ...
        let (files, _) = parse_unified_diff_lines(lines, DiffFormat::GitStyle).unwrap();
